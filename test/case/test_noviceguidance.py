...
from realization.test.url.url_noviceguidance import TestNoviceG
from realization.tool.my_files import Testfile
from realization.comm.asserts.my_error import TestError


@allure.epic("新手引导-大数据")
class TestNoviceGs(TestPublicLibrary, TestNoviceG, TestAssert, jsonpAtLibrary, TestTime, Testfile, TestError):
    @allure.feature("新手引导-大数据-查询")
    @allure.title('查询-用例1-正向')
    @pytest.mark.flaky(reruns=2, reruns_delay=2.5)
    def testcase_01(self):
        urls = self.base_urls + self.url_NoviceG_select
        r = self.requests(method='post', url=urls, data=self.data_NoviceG_select, headers=self.fromHeader())
    ...
    @allure.feature("新手引导-大数据-新手引导柱形图表")
    @allure.title('新手引导柱形图表-用例1-正向')
    # @pytest.mark.flaky(reruns=2, reruns_delay=2.5)
    def testcase_03(self):
        urls = self.base_urls + self.url_NoviceG_from
        r = self.requests(method='post', url=urls, data=self.data_NoviceG_from, headers=self.fromHeader())
        # pytest.xfail(reason="此接口到调试")


if __name__ == '__main__':
    pytest.main(['-vs', 'test_noviceguidance.py'])
